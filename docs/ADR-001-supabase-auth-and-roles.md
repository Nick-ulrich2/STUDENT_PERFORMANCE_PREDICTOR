...
**Incident resolu (2026-09-22/23)** : sur le projet Supabase reel utilise pour le developpement, `SELECT * from public.profiles` levait `infinite recursion detected in policy for relation "profiles"` (Postgres `42P17`), cassant `/me/profile-attributes`, `/me/features` et donc `/predict/from-activity` pour tous les utilisateurs. Cause reelle, diagnostiquee via `select policyname, cmd, qual from pg_policies where tablename = 'profiles'` : une policy heritee `profiles_select_admin`, jamais creee par nos migrations et jamais nommee dans leurs `drop policy if exists`, restait active avec la definition `EXISTS (SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.role = 'admin')` — le pattern recursif classique (une policy sur `profiles` qui re-interroge `profiles`). Une seule policy recursive suffit a casser tout `SELECT`, meme quand toutes les autres (dont `profiles_admin_can_read_all`, correcte) lisent `auth.jwt()` sans sous-requete. Corrige en supprimant manuellement `profiles_select_admin` (et le doublon inoffensif `profiles_select_own`) via le SQL Editor. Si ce symptome reapparait sur un autre projet Supabase, executer d'abord la requete `pg_policies` ci-dessus pour reperer toute policy recursive heritee (creee manuellement via le dashboard avant l'adoption de ces migrations) avant de supposer que les migrations n'ont pas ete executees. `app/test_rls_integration.py` rejoue l'integralite des 8 migrations sur un Postgres vierge et confirme qu'elles ne creent, elles, aucune policy recursive.
